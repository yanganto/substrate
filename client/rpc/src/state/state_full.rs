    ...
    Client::Api: Metadata<Block, Error = sp_blockchain::Error>,
{
    ...
    fn query_state_root_at(&self, at: Option<Block::Hash>) -> FutureResult<Block::Hash> {
        let at = at.unwrap_or_else(|| self.client.info().best_hash);
        if let Ok(state_root) = self.client.state_root(BlockId::Hash(at)) {
            if let Some(h) = state_root {
                return Box::new(result(Ok(h)));
            }
        }
        Box::new(result(Ok(Block::Hash::default())))
    }

    fn query_extrinsics_root_at(&self, at: Option<Block::Hash>) -> FutureResult<Block::Hash> {
        let at = at.unwrap_or_else(|| self.client.info().best_hash);
        if let Ok(extrinsics_root) = self.client.extrinsics_root(BlockId::Hash(at)) {
            if let Some(h) = extrinsics_root {
                return Box::new(result(Ok(h)));
            }
        }
        Box::new(result(Ok(Block::Hash::default())))
