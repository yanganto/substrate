    ...
    F: Fetcher<Block> + 'static,
{
    ...
    fn storage_hash(
        ...
        Box::new(
            StateBackend::storage(self, block, key).and_then(|maybe_storage| {
                result(Ok(
                    maybe_storage.map(|storage| HashFor::<Block>::hash(&storage.0))
                ))
            }),
        )
    }
    ...
    fn query_state_root_at(&self, _at: Option<Block::Hash>) -> FutureResult<Block::Hash> {
        Box::new(result(Err(client_err(
            ClientError::NotAvailableOnLightClient,
        ))))
    }

    fn query_extrinsics_root_at(&self, _at: Option<Block::Hash>) -> FutureResult<Block::Hash> {
        Box::new(result(Err(client_err(
            ClientError::NotAvailableOnLightClient,
        ))))
    }
    ...
    F: Fetcher<Block> + 'static,
{
    ...
    fn storage_hash(
        ...
        Box::new(
            ChildStateBackend::storage(self, block, storage_key, key).and_then(|maybe_storage| {
                result(Ok(
                    maybe_storage.map(|storage| HashFor::<Block>::hash(&storage.0))
                ))
            }),
