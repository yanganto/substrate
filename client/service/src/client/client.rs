...
use {
    ...
    sc_client_api::{in_mem, CloneableSpawn},
    ...
    RA: Send + Sync,
{
    ...
    fn state_root(&self, id: BlockId<Block>) -> sp_blockchain::Result<Option<Block::Hash>> {
        match self.backend.blockchain().header(id) {
            Ok(may_header) => {
                if let Some(h) = may_header {
                    return Ok(Some(*h.state_root()));
                } else {
                    return Ok(None);
                }
            }
    ...
    fn extrinsics_root(&self, id: BlockId<Block>) -> sp_blockchain::Result<Option<Block::Hash>> {
        match self.backend.blockchain().header(id) {
            Ok(may_header) => {
                if let Some(h) = may_header {
                    return Ok(Some(*h.extrinsics_root()));
                } else {
                    return Ok(None);
                }
            }
