...
/// Read a state_root from the database.
pub fn read_state_root<Block: BlockT>(
    db: &dyn Database<DbHash>,
    col_index: u32,
    col: u32,
    id: BlockId<Block>,
    ...
    match read_db(db, col_index, col, id)? {
        Some(header) => match Block::Header::decode(&mut &header[..]) {
            Ok(header) => Ok(Some(*header.state_root())),
            Err(_) => {
                return Err(sp_blockchain::Error::Backend(
                    "Error decoding header".into(),
                ))
...
/// Read a extrinsics_root from the database.
pub fn read_extrinsics_root<Block: BlockT>(
    db: &dyn Database<DbHash>,
    col_index: u32,
    col: u32,
    id: BlockId<Block>,
    ...
    match read_db(db, col_index, col, id)? {
        Some(header) => match Block::Header::decode(&mut &header[..]) {
            Ok(header) => Ok(Some(*header.extrinsics_root())),
            Err(_) => {
                return Err(sp_blockchain::Error::Backend(
                    "Error decoding header".into(),
                ))
