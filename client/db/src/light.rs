...
use crate::utils::{
    self, block_id_to_lookup_key, meta_keys, read_db, read_meta, DatabaseType, Meta,
};
use crate::{DatabaseSettings, DbHash, FrozenForDuration};
...
pub(crate) mod tests {
    ...
    use sp_runtime::generic::{ChangesTrieSignal, DigestItem};
    use sp_runtime::testing::{Block as RawBlock, ExtrinsicWrapper, Header, H256 as Hash};
    ...
    fn finalized_ancient_headers_are_replaced_with_cht() {
        ...
        assert_eq!(
            raw_db.count(columns::KEY_LOOKUP),
        ...
        assert!(db
            ...
            .is_none());
        assert!(db
            .changes_trie_cht_root(cht_size, cht_size / 2)
            .unwrap()
            .is_some());
        assert!(db
            .changes_trie_cht_root(cht_size, cht_size + cht_size / 2)
    ...
    fn authorities_are_cached() {
        ...
        let (hash7, hash8, hash6_1, hash6_2, hash6_1_1, hash6_1_2) = {
            ...
            assert_eq!(
                authorities(db.cache(), BlockId::Hash(hash7)),
                Some(vec![auth3()])
            );
            ...
            assert_eq!(
                authorities(db.cache(), BlockId::Hash(hash8)),
                Some(vec![auth3()])
            );
            ...
            assert_eq!(
                authorities(db.cache(), BlockId::Hash(hash6_1)),
                Some(vec![auth4()])
            );
            ...
            assert_eq!(
                authorities(db.cache(), BlockId::Hash(hash6_1_1)),
                Some(vec![auth5()])
            );
            ...
            assert_eq!(
                authorities(db.cache(), BlockId::Hash(hash6_1_2)),
                Some(vec![auth6()])
            );
            ...
            assert_eq!(
                authorities(db.cache(), BlockId::Hash(hash6_2)),
                Some(vec![auth4()])
            );
    ...
    fn changes_trie_configuration_is_tracked_on_light_client() {
        ...
        let hash0 = insert_block(&db, HashMap::new(), || {
            default_header(&Default::default(), 0)
        });
        assert_eq!(
            db.cache()
