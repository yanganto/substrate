...
use codec::Encode;
use sr_primitives::app_crypto::{self, RuntimeAppPublic, AppPublic};
...
impl<Public, Signature, AnyAppPublic> Signer<Public, Signature> for AnyAppPublic where
	AnyAppPublic: RuntimeAppPublic
		+ app_crypto::AppPublic
		+ From<<AnyAppPublic as app_crypto::AppPublic>::Generic>,
	<AnyAppPublic as RuntimeAppPublic>::Signature: app_crypto::AppSignature,
	Signature: From<
		<<AnyAppPublic as RuntimeAppPublic>::Signature as app_crypto::AppSignature>::Generic
	>,
	Public: rstd::convert::TryInto<<AnyAppPublic as app_crypto::AppPublic>::Generic>
{
	fn sign<Payload: Encode>(public: Public, raw_payload: &Payload) -> Option<Signature> {
		raw_payload.using_encoded(|payload| {
			...
			AnyAppPublic::from(public).sign(&payload)
				.map(
					<<AnyAppPublic as RuntimeAppPublic>::Signature as app_crypto::AppSignature>
					 ::Generic::from
				)
				.map(Signature::from)
		})
...
pub trait CreateTransaction<T: crate::Trait, Extrinsic: ExtrinsicT> {
	...
	type Public: Clone + sr_primitives::traits::IdentifyAccount<AccountId=T::AccountId>; // TODO: undo these or ask @todr
	...
	fn into_account(key: Self::Public) -> T::AccountId {
		key.into_account()
	}
}
...
/// A default type used to submit transactions to the pool. This is passed into each runtime as an
/// opaque associated type that can have either of:
/// - [`SubmitSignedTransaction`]
/// - [`SubmitUnsignedTransaction`]
/// - [`RuntimeIdentifyAccount`]
...
pub trait RuntimeIdentifyAccount<T: crate::Trait> {
	type Key;
	fn into_account(key: Self::Key) -> T::AccountId;
}

impl<T, C, E, S, Call> RuntimeIdentifyAccount<T> for TransactionSubmitter<S, C, E> where
	T: crate::Trait,
	S: Signer<<C as CreateTransaction<T, E>>::Public, <C as CreateTransaction<T, E>>::Signature>,
	E: ExtrinsicT<Call=Call> + codec::Encode,
	C: CreateTransaction<T, E>,
{
	type Key = PublicOf<T, Call, Self>;
	fn into_account(key: Self::Key) -> T::AccountId {
		<<C as CreateTransaction<T, E>>::Public as IdentifyAccount>::into_account(key)
	}
}
